...
8. On a generated graph, run **Expand this company** and confirm the next immutable version opens automatically, preserves the existing node positions and viewport, and hides its newly added officers by default. Reveal and hide them from the same company and confirm that no organisation address or onward relationship was added by the direct expansion.
