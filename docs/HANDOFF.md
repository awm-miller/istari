...
8. On a generated graph, run **Expand one round from here** on a company and confirm the next immutable version hides its added nodes by default. Reveal and hide them from the centre node, then use **Configure custom enrichment** to test multiple centres and missing-PDF-only backfill without changing the prior version.
