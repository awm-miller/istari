...
8. On a generated graph, run **Expand this company** and confirm the next immutable version opens automatically, preserves collision-free node positions and the viewport, and shows its newly added officers by default. Hide and show the latest round from the same company, confirm the person-resolution feedback, and confirm that no organisation address or onward relationship was added by the direct expansion.
